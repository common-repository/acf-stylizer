 ...
 */
abstract class ACF_Stylizer_Field extends ACF_Stylizer_Wrapper
{
	/**
	...
	public function __construct($properties = array())
	{
		...
		{
			$this->{$name} = ACF_Stylizer_Utilities::check_array($this->{$name});
		}
	}
		...
		switch ($name)
		{
			...
			case 'input_attributes':
				...
				if (!empty($this->id))
				{
					...
					? ' id="acfs-' . $this->id . '" name="' . $this->id . '"'
					: ' data-acfs-identifier="' . $this->id . '"';
				}
			...
			case 'label_attribute':
				...
				? ' for="acfs-' . $this->id . '"'
				: ' data-acfs-identifier="acfs-' . $this->id . '"';
			...
			case 'sanitization':
				...
				return ACF_Stylizer_Sanitization::TEXT;
	...
	protected function _output($field, $type, $echo)
	{
		...
		if (!empty($field))
		{
			...
			$condition_attr = ' data-acfs-field="';
			$condition_replacement = $condition_attr . '__f__"';
			
			$this->push('wrapper_classes', 'acfs-field');
			$this->push('wrapper_classes', 'acfs-field-' . sanitize_key($type));
			
			if ($this->is_template)
			{
				$this->push('wrapper_classes', 'acfs-field-template');
			}
			...
			if (!$this->hide_labels)
			{
				...
				{
					$description = '<div class="acfs-description">'
				...
				if (!empty($this->label))
				{
					$label = '<div class="acfs-field-label">'
			...
			if (!empty($this->conditions))
			{
				foreach ($this->conditions as $condition)
				{
					...
					(
						...
						&&
						ACF_Stylizer_Utilities::starts_with('ACF_Stylizer_Field_', get_class($condition['field']))
						&&
					...
					)
					{
						...
						$condition_output = '<div class="acfs-hidden acfs-condition" data-acfs-compare="' . esc_attr($condition['compare']) . '" data-acfs-conditional="' . $this->id . '"' . $condition_replacement . ' data-acfs-value="' . esc_attr($condition['value']) . '"></div>';
			...
			{
				$output .= '<div class="acfs-clear"></div>';
			}
		}
	...
	/**
	 * Add one or more child fields to the field.
	 *
	 * @since 2.0.0
	 *
	 * @access public
